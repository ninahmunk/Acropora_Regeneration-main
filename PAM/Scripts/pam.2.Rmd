...
mod<- lmer(fv_fm ~ wound*temp*as.factor(timepoint) + (1|genotype/coral_id), data = data.filtered%>%filter(!timepoint == 0), contrasts = list(wound = "contr.sum", temp = "contr.poly"))
anova(mod, type = 3)
...
```
# figure based on model: temp*timepoint significant 
```{r}

mod<- lmer(fv_fm ~ wound*temp*timepoint + (1|genotype/coral_id), data = data.filtered, contrasts = list(wound = "contr.sum", temp = "contr.poly"))
anova(mod, type = 3)
...
ggplot(data = data.filtered, aes(x = timepoint, y = fv_fm, col = temp))+geom_smooth(method = "lm")+
  ...
  scale_color_manual(values = c('blue', 'red'), labels = c("Ambient (~28)", "Elevated (~30)"))+
   theme_few(base_size = 15) -> fvfm

ggsave("fvfm.png", plot = fvfm, path = here(),
       width = 10,
       height = 8,
       units = "in")
```
...
```{r}
ggplot(data = data.filtered%>%filter(!timepoint ==0), aes(x = as.factor(timepoint), y = fv_fm, col = temp))+
  #geom_jitter(size = .5, alpha = .5)+
  stat_summary(fun.data = mean_se, geom = "errorbar", width = 0.2, position = position_dodge(width = 0.2)) +
  stat_summary( fun = "mean", geom = "point", size = 3, position = position_dodge(width = 0.2))+ 
  #stat_summary( fun = "mean", geom = "line", size = 1, position = position_dodge(width = 0.2), linetype = "dashed", aes(group = temp))+
