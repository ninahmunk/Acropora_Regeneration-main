...
anova(mod, type = 3)
#reduced
mod<- lmer(umol.cm2.hr ~ wound*temp + temp*timepoint + (1|genotype/coral_id), data = rates.filtered%>%filter(rate == "Respiration")%>%filter(!timepoint == 0), contrasts = list(wound = "contr.sum", temp = "contr.poly"))
...
anova(mod, type = 3) # significant 3way interaction between temp, time, and wound, also sig interaction between temp and time
...
anova(mod, type = 3) # temp is highly significant and temp*wound interaction highly significant
...
anova(mod, type = 3) # timepoint is significant
...
```{r}
ggplot(data = rates.filtered, aes(x = timepoint, y = umol.cm2.hr, col = temp))+
