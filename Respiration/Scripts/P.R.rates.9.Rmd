...
anova(mod, type = 3)
```
Respiration figure that pairs with result from final model. 
```{r}
...
ggplot(data = resp.filtered, aes(x = timepoint, y = umol.cm2.hr, col = temp))+
 ...
 # stat_summary( fun = "mean", geom = "point", size = 2, position = position_dodge(width = 0.4))+ 
  ...
  geom_smooth(data=resp.filtered, aes(x=timepoint, y=umol.cm2.hr), alpha=0.3, method = "lm")+
...
#        units = "in")
```
I want to examine differences in respiration rates at each timepoint
```{r}
...
model_time2 <- lmer(umol.cm2.hr ~ temp*wound + (1|genotype), data=time2, contrasts = list(wound = "contr.sum", temp = "contr.poly"))
summary(model_time2)
Anova(model_time2)
...
ggplot(resp.filtered, aes(x=timepoint, y=umol.cm2.hr, color=temp)) + 
  stat_summary(fun=mean, geom="point", aes(group=temp), size=5)+
  stat_summary(fun.data = mean_se, geom = "errorbar", width = 0.2, position = position_dodge(width = 0.4))

ggplot(resp.filtered, aes(x=timepoint, y=umol.cm2.hr, color=wound)) + 
  stat_summary(fun=mean, geom="point", aes(group=wound), size=5)+
  stat_summary(fun.data = mean_se, geom = "errorbar", width = 0.2, position = position_dodge(width = 0.4))
...
ggplot(data = netphoto.filtered, aes(x = timepoint, y = umol.cm2.hr, shape = temp, col = temp))+
 ...
 scale_shape_manual(values = c(1, 19), labels = c("Ambient", "Elevated"))+
  ...
  theme_base()+
   theme_few(base_size = 20)+
  theme(panel.spacing=unit(1,"lines"))+
  theme(legend.position = "none")-> net.photo
 

# ggsave("net.photo.png", plot = net.photo, path = here(),
#        width = 10,
#        height = 8,
#        units = "in")

```
Examining differences in net photosynthetic rates at each timepoint
```{r}
# Subset data for a single time point
...
model_time3 <- lmer(umol.cm2.hr ~ temp*wound + (1|genotype), data=time3, contrasts = list(wound = "contr.sum", temp = "contr.poly"))
summary(model_time3)
Anova(model_time3)
# Post-hoc tests
...
ggplot(netphoto.filtered, aes(x=timepoint, y=umol.cm2.hr, color=temp)) + 
  stat_summary(fun=mean, geom="point", aes(group=temp), size=5)+
  stat_summary(fun.data = mean_se, geom = "errorbar", width = 0.2, position = position_dodge(width = 0.4))+ facet_wrap(~wound)

ggplot(netphoto.filtered, aes(x=timepoint, y=umol.cm2.hr, color=wound)) + 
  stat_summary(fun=mean, geom="point", aes(group=wound), size=5)+
  stat_summary(fun.data = mean_se, geom = "errorbar", width = 0.2, position = position_dodge(width = 0.4))
...
ggplot(data = netphoto.filtered, aes(x = timepoint, y = umol.cm2.hr, shape = temp, col = temp))+
  stat_summary(fun=mean, geom="point", aes(group=temp), size = 3, position = position_dodge(width = 0.3))+
  stat_summary(fun.data = mean_se, geom = "errorbar", aes(group=temp), width = 0.2, position = position_dodge(width = 0.3))+
  ylab("Net Photosynthesis (umol/cm2/hr)")+
  xlab(NULL)+
  facet_grid(~wound, labeller = labeller(wound = c("0" = "No Injury", "1" = "Fragmentation", "2" = "Abrasion")), scales = "free")+ 
  labs(fill = "Temperature")+
 # scale_x_continuous(labels = c("1", "10", "19"))+
 scale_shape_manual(values = c(1, 19), labels = c("Ambient", "Elevated"))+
  scale_color_manual(values = c('blue', 'red'), labels = c("Ambient", "Elevated"))+
  guides(col = guide_legend("Temperature"), shape = guide_legend("Temperature"))+
  theme_base()
...
ggplot(data = grossphoto.filtered, aes(x = timepoint, y = umol.cm2.hr))+
  ...
  ylab("Gross Photosynthesis (umol/cm2/hr)")+
  xlab("Day")+
 # facet_grid(~wound, labeller = labeller(wound = c("0" = "No Injury", "1" = "Fragmentation", "2" = "Abrasion")), scales = "free_y", space="free")+ 
  theme_classic()+
  #labs(color = "Temperature")+
...
ggplot(data = ratio.filtered, aes(x = timepoint, y = ratio, col = temp, group = temp, shape = temp))+
 ...
 facet_grid(~wound, labeller = labeller(wound = c("0" = "No Injury", "1" = "Fragmentation", "2" = "Abrasion")), scales = "free_y", space="free")+ 
  ...
  theme(strip.text = element_blank())-> PR
...
# Mixed-effects model for a single time point
model_time1 <- lmer(ratio ~ temp*wound + (1|genotype), data=time1, contrasts = list(wound = "contr.sum", temp = "contr.poly"))
summary(model_time1)
# ANOVA on the mixed model
Anova(model_time1)

time2 <- ratio.filtered%>%filter(timepoint == 10)
model_time2 <- lmer(ratio ~ temp*wound + (1|genotype), data=time2, contrasts = list(wound = "contr.sum", temp = "contr.poly"))
summary(model_time2)
Anova(model_time2)
...
model_time3 <- lmer(ratio ~ temp*wound + (1|genotype), data=time3, contrasts = list(wound = "contr.sum", temp = "contr.poly"))
summary(model_time3)
Anova(model_time3)
emmeans(model_time3, pairwise ~ temp*wound) # no significant pairwise comparisons

ggplot(time3, aes(x=temp, y=ratio, color=temp)) + 
  stat_summary(fun=mean, geom="point", aes(group=temp), size=5)+
  stat_summary(fun.data = mean_se, geom = "errorbar", width = 0.2, position = position_dodge(width = 0.4)) +facet_grid(~wound)
```
